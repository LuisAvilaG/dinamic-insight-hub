...
import { TrendingUp, BarChart, Table, LineChart, PieChart, ArrowLeft, Loader2 } from 'lucide-react';
...
export const NewAddWidgetDialog = (props: NewAddWidgetDialogProps) => {
  ...
  const handleConfigChange = (newValues: any) => {
    const newConfig = { ...widgetConfig, ...newValues };
    const { schema, table, columns } = newConfig;

    if (schema && table && columns && columns.length > 0) {
        const query = `SELECT ${columns.map(c => `"${c}"`).join(', ')} FROM "${schema}"."${table}"`;
        setWidgetConfig({ ...newConfig, query });
    } else {
        setWidgetConfig({ ...newConfig, query: null });
    }
  };
...
const DataTableConfigScreen = ({ config, onConfigChange }: { config: any; onConfigChange: (c: any) => void }) => {
    return (
...
// ... (KpiConfigScreen, BarChartConfigScreen, and their previews remain the same)
